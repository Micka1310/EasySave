    ...
    {
        {
            Lang.FR, new Dictionary<string, string>
            {
                ...
                { "wpf_section_theme", "Apparence" },
                { "wpf_theme_dark", "Sombre" },
                { "wpf_theme_light", "Clair" },
                { "wpf_settings_theme", "Thème de l'interface" },
                { "wpf_settings_theme_hint", "Basculez entre le mode sombre (confort visuel) et le mode clair (environnement lumineux)." },
                ...
                { "wpf_settings_logs_file", "Fichier de log actif" },
                { "wpf_settings_logs_preview", "Aperçu des derniers logs" },
                { "wpf_settings_logs_refresh", "Rafraichir l’aperçu" },
                { "wpf_settings_open_logs_folder", "Ouvrir le dossier des logs" },
                { "wpf_settings_section_log_routing", "Centralisation des journaux (Docker)" },
                { "wpf_settings_log_destination", "Destination des journaux" },
                { "wpf_settings_central_log_url", "URL du service LogCentral" },
                { "wpf_settings_central_log_hint", "Local : uniquement sur ce PC (%AppData%\\EasySave). Central : uniquement le conteneur Docker (un fichier .ndjson par jour, toutes machines ; chaque ligne indique utilisateur et poste). Both : les deux. Pour tester sans serveur distant : installez Docker Desktop, puis à la racine du projet : docker compose up -d --build — URL : http://localhost:5088 — fichiers dans le dossier central-logs." },
                { "wpf_settings_section_business_software", "Logiciel métier" },
                { "wpf_settings_business_software_label", "Logiciels à surveiller (ex: notepad; calc)" },
                { "wpf_settings_business_software_hint", "Si un logiciel métier est détecté, le démarrage est bloqué. En séquentiel, la sauvegarde termine le fichier courant puis se met en pause." },
                { "wpf_settings_business_software_add_button", "Ajouter" },
                { "wpf_settings_business_software_list", "Logiciels surveillés" },
                { "wpf_settings_business_software_reset_defaults", "Restaurer notepad + calc" },
                { "wpf_settings_business_software_clear_all", "Vider la liste" },
                { "wpf_settings_business_software_empty", "Aucun logiciel surveillé. Ajoutez-en au moins un pour activer la protection." },
                { "wpf_business_software_blocked_start", "Démarrage interdit : logiciel métier détecté." },
                { "wpf_business_software_pause", "Sauvegarde en pause : logiciel métier détecté." },
                { "wpf_business_software_resume", "Sauvegarde reprise : logiciel métier fermé." },
                { "wpf_business_software_log", "Arrêt/pause déclenché par logiciel métier" },
                { "wpf_settings_section_priority", "Fichiers prioritaires" },
                { "wpf_settings_priority_extensions", "Extensions prioritaires (copiées en premier)" },
                { "wpf_settings_priority_selected", "Extensions prioritaires sélectionnées" },
                { "wpf_settings_priority_add", "Ajouter une extension personnalisée" },
                { "wpf_settings_priority_add_button", "Ajouter" },
                { "wpf_settings_priority_hint", "Cochez les extensions à traiter en premier. Tant qu'il reste des fichiers de ces types en attente sur au moins un travail, les fichiers non prioritaires ne sont pas copiés." },
                { "wpf_settings_section_bandwidth", "Bande passante" },
                { "wpf_settings_bandwidth_threshold", "Taille au-dessus de laquelle un fichier est considéré volumineux" },
                { "wpf_settings_bandwidth_threshold_unit", "Unité" },
                { "wpf_settings_bandwidth_hint", "Deux fichiers volumineux (> seuil) ne peuvent pas être transférés en même temps par des travaux parallèles. Les fichiers plus petits continuent en parallèle. Mettez la valeur à 0 pour désactiver la règle." },
                { "wpf_settings_bandwidth_disabled", "Règle désactivée." },
                { "wpf_settings_section_encryption", "Chiffrement CryptoSoft" },
                { "wpf_settings_encryption_extensions", "Extensions à chiffrer" },
                { "wpf_settings_encryption_selected", "Extensions sélectionnées" },
                { "wpf_settings_encryption_add", "Ajouter une extension personnalisée" },
                { "wpf_settings_encryption_add_button", "Ajouter" },
                ...
                { "wpf_settings_about_body", "EasySave v3 — interface graphique. Journaux local, centralisés (Docker) ou les deux. Les travaux sont enregistrés dans works.json." },
        ...
        },
        {
            Lang.EN, new Dictionary<string, string>
            {
                ...
                { "wpf_section_theme", "Appearance" },
                { "wpf_theme_dark", "Dark" },
                { "wpf_theme_light", "Light" },
                { "wpf_settings_theme", "Interface theme" },
                { "wpf_settings_theme_hint", "Switch between dark mode (low glare) and light mode (bright environments)." },
                ...
                { "wpf_settings_logs_file", "Current log file" },
                { "wpf_settings_logs_preview", "Latest logs preview" },
                { "wpf_settings_logs_refresh", "Refresh preview" },
                { "wpf_settings_open_logs_folder", "Open logs folder" },
                { "wpf_settings_section_log_routing", "Log centralization (Docker)" },
                { "wpf_settings_log_destination", "Log destination" },
                { "wpf_settings_central_log_url", "LogCentral service URL" },
                { "wpf_settings_central_log_hint", "Local: this PC only (%AppData%\\EasySave). Central: Docker container only (one .ndjson file per day; each line has user and machine). Both: both. To test without a remote server: install Docker Desktop, then at project root: docker compose up -d --build — URL: http://localhost:5088 — files in central-logs folder." },
                { "wpf_settings_section_business_software", "Business software" },
                { "wpf_settings_business_software_label", "Software to monitor (e.g. notepad; calc)" },
                { "wpf_settings_business_software_hint", "If business software is detected, start is blocked. In sequential runs, backup finishes current file then pauses." },
                { "wpf_settings_business_software_add_button", "Add" },
                { "wpf_settings_business_software_list", "Monitored software" },
                { "wpf_settings_business_software_reset_defaults", "Restore notepad + calc" },
                { "wpf_settings_business_software_clear_all", "Clear list" },
                { "wpf_settings_business_software_empty", "No monitored software. Add at least one to enable protection." },
                { "wpf_business_software_blocked_start", "Start blocked: business software detected." },
                { "wpf_business_software_pause", "Backup paused: business software detected." },
                { "wpf_business_software_resume", "Backup resumed: business software closed." },
                { "wpf_business_software_log", "Stop/pause triggered by business software" },
                { "wpf_settings_section_priority", "Priority files" },
                { "wpf_settings_priority_extensions", "Priority extensions (copied first)" },
                { "wpf_settings_priority_selected", "Selected priority extensions" },
                { "wpf_settings_priority_add", "Add a custom extension" },
                { "wpf_settings_priority_add_button", "Add" },
                { "wpf_settings_priority_hint", "Check extensions to copy first. While priority files are still pending on any job, non-priority files are not copied." },
                { "wpf_settings_section_bandwidth", "Bandwidth" },
                { "wpf_settings_bandwidth_threshold", "Size above which a file is considered large" },
                { "wpf_settings_bandwidth_threshold_unit", "Unit" },
                { "wpf_settings_bandwidth_hint", "Two large files (> threshold) cannot be transferred simultaneously by parallel jobs. Smaller files keep running in parallel. Set the value to 0 to disable the rule." },
                { "wpf_settings_bandwidth_disabled", "Rule disabled." },
                { "wpf_settings_section_encryption", "CryptoSoft encryption" },
                { "wpf_settings_encryption_extensions", "Extensions to encrypt" },
                { "wpf_settings_encryption_selected", "Selected extensions" },
                { "wpf_settings_encryption_add", "Add a custom extension" },
                { "wpf_settings_encryption_add_button", "Add" },
                ...
                { "wpf_settings_about_body", "EasySave v3 — graphical UI. Local, centralized (Docker), or both log modes. Jobs are stored in works.json." },
